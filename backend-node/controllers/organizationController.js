...
export const getInstitutions = async (req, res) => {
 ...
 export const getStudents = async (req, res) => {
  try {
    ...
    // Sorting
    const sort = {};
    if (sortBy === 'analysisResumeScore') {
      sort.analysisResumeScore = sortOrder === 'desc' ? -1 : 1;
    }
    ...
    const students = await Individual.find(query)
      .select('name department year resumeFilePath analysisResumeScore analysisFilePath reportFilePath college')
    ...
    const formattedStudents = students.map((student) => ({
      ...
      institution: student.college ? student.college.name : 'Unknown',
      resumeScore: student.analysisResumeScore || 0,
      resumeUrl: student.resumeFilePath || '',
      hasReport: !!student.reportFilePath,
      analysisFilePath: student.analysisFilePath || undefined,
    }));
